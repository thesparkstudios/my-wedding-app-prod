...
export default function App() {
  ...
  const generateQuoteLink = async () => {
    // Extra diagnostics to help you see what's wrong on-screen & in console
    console.log("[GEN] userId:", userId, " db:", !!db);
    if (!client.name) { setError("Client name is required."); return; }
    if (!auth || !db) { setError("Firebase not initialized. Check env/__firebase_config."); return; }
    if (!userId) { setError("Not authenticated yet. Wait a second or ensure Anonymous Auth is enabled in Firebase Console."); return; }
    ...
    try {
      ...
      console.log("[GEN] Quote saved:", docRef.id, "URL:", url);
      window.open(url, "_blank");
      showMsg("Quote link generated and opened.");
    } catch (err) {
      console.error("[GEN] Firestore write failed:", err);
      // Surface Firebase error codes nicely
      const code = err?.code || "unknown";
      if (code.includes("permission-denied")) {
        setError("Permission denied: Check Firestore Security Rules to allow writes for the signed-in user.");
      } else if (code.includes("failed-precondition")) {
        setError("Firestore not enabled or indexes missing. Ensure Firestore is set up in the Firebase Console.");
      } else {
        setError(`Failed to generate link: ${err.message || err}`);
      }
        ...
        <Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 text-sm">
            <div className="rounded-lg border border-neutral-800 bg-neutral-900/40 p-3">
              <div className="text-neutral-400">Auth</div>
              <div className="font-mono text-emerald-400">{userId ? `OK (${userId.slice(0,6)}…)` : "waiting / disabled"}</div>
            </div>
            <div className="rounded-lg border border-neutral-800 bg-neutral-900/40 p-3">
              <div className="text-neutral-400">Firestore</div>
              <div className="font-mono text-emerald-400">{db ? "initialized" : "not ready"}</div>
            </div>
            <div className="rounded-lg border border-neutral-800 bg-neutral-900/40 p-3">
              <div className="text-neutral-400">App ID</div>
              <div className="font-mono">{appId}</div>
            </div>
            <div className="rounded-lg border border-neutral-800 bg-neutral-900/40 p-3">
              <div className="text-neutral-400">Project</div>
              <div className="font-mono truncate">{(typeof __firebase_config !== 'undefined' && __firebase_config) ? JSON.parse(__firebase_config).projectId : process.env.REACT_APP_FIREBASE_PROJECT_ID || "—"}</div>
            </div>
          </div>
          <div className="mt-3 text-xs text-neutral-400">If Auth is "waiting", enable Anonymous auth in Firebase Console → Authentication. If Firestore is "not ready", enable Firestore and set rules (see chat instructions).</div>
        </Card>
