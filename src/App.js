      ...
      {view === 'editor' && (
        <div className="max-w-6xl mx-auto py-12 md:py-20 px-6 font-sans text-slate-900 pb-48 leading-relaxed">
          ...
          <div className="space-y-12">
                ...
                {proposalData.days.map((day) => (
                  <div key={day.id} className={`p-8 rounded-[2.5rem] border-2 transition-all duration-500 ${day.highlight ? 'border-indigo-100 bg-indigo-50/20' : 'border-slate-100 bg-white'}`}>
                    ...
                    <div className="space-y-4 font-sans">
                      ...
                      <label className="flex items-center gap-3 mt-6 cursor-pointer select-none px-1 font-sans"><input type="checkbox" checked={day.highlight} onChange={(e) => updateDay(day.id, 'highlight', e.target.checked)} className="w-5 h-5 rounded border-slate-200 text-slate-950 focus:ring-slate-950 font-sans" /><span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest font-sans">Full Day Coverage</span></label>
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  <div key={pkg.id} className={`p-8 md:p-12 rounded-[3.5rem] border-2 transition-all duration-500 ${pkg.isVisible ? 'bg-white border-slate-200 shadow-sm' : 'bg-slate-50/50 opacity-50 border-dashed border-slate-200'}`}>
                    ...
                    {pkg.isVisible && (
                      <div className="grid md:grid-cols-2 gap-12 font-sans">
                        ...
                        <div className="flex flex-col gap-2 font-sans"><label className="text-[11px] font-bold text-slate-400 uppercase tracking-[0.2em] font-sans ml-1 font-sans">Collection Specifics</label><textarea value={pkg.features.join('\n')} onChange={(e) => updatePackageFeatures(pkg.id, e.target.value.split('\n'))} className="p-6 border border-slate-100 bg-slate-50 rounded-2xl text-sm h-full min-h-[250px] leading-loose text-slate-900 font-medium font-sans" /></div>
                      </div>
                    )}
                  </div>
                ))}
            ...
            <section className="font-sans">
              <div className="flex justify-between items-center mb-10 px-4 font-sans"><h2 className="text-xs font-black flex items-center gap-3 text-slate-400 uppercase tracking-[0.3em] font-sans">04. Featured Studio Work</h2><button onClick={addWorkLink} className="text-[10px] bg-slate-950 text-white px-6 py-3 rounded-full font-bold uppercase tracking-widest shadow-md font-sans"><Plus size={14} /> Add Link</button></div>
                ...
                {proposalData.workLinks?.map((link) => (
                  <div key={link.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 relative group font-sans">
                    ...
                    <div className="space-y-4 font-sans">
                      ...
                      <div className="flex flex-col gap-2 font-sans"><label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest font-sans">URL (YouTube/Website)</label><input type="text" value={link.url} onChange={(e) => updateWorkLink(link.id, 'url', e.target.value)} className="w-full p-3 bg-slate-50/50 rounded-xl text-xs outline-none font-sans" /></div>
            ...
            <section className="font-sans">
              <div className="flex justify-between items-center mb-10 px-4 font-sans"><h2 className="text-xs font-black flex items-center gap-3 text-slate-400 uppercase tracking-[0.3em] font-sans">05. Client Praise (Google Reviews)</h2><button onClick={addReview} className="text-[10px] bg-slate-950 text-white px-6 py-3 rounded-full font-bold uppercase tracking-widest shadow-md font-sans"><Plus size={14} /> Add Review</button></div>
              <div className="grid md:grid-cols-2 gap-6 font-sans">
            ...
            <>
              ...
              <div className="relative h-[80vh] md:h-screen flex items-center justify-center overflow-hidden bg-[#0d0d0d]">
                ...
                <div className="relative z-10 text-center text-white px-8 font-sans">
                  ...
                  <div className="max-w-lg mx-auto border-t border-white/20 pt-12 font-sans"><p className="text-sm md:text-xl font-light tracking-[0.2em] uppercase opacity-90 font-sans">Bespoke Cinematic Narrative</p></div>
                </div>
                <div className="absolute bottom-16 left-1/2 -translate-x-1/2 animate-bounce opacity-40 font-sans"><div className="w-[1px] h-20 bg-white"></div></div>
              </div>
              ...
              <section className="max-w-5xl mx-auto py-40 md:py-64 px-8 text-center leading-relaxed">
                ...
                <p className="text-2xl md:text-5xl lg:text-6xl leading-[1.6] text-[#222222] font-light italic px-4 font-serif">"{proposalData.visionStatement}"</p>
              </section>
                  ...
                  <div className={`grid gap-8 md:gap-10 font-sans ${
                    ...
                    {proposalData.days.map((day) => (
                      <div key={day.id} className={`relative p-8 md:p-10 bg-white rounded-[2.5rem] shadow-sm border border-slate-100 transition-all duration-700 hover:shadow-2xl hover:-translate-y-4 ${day.highlight ? 'ring-1 ring-[#C5A059]/30' : ''} font-sans`}>
                        ...
                        <h4 className="font-black text-[11px] font-sans uppercase tracking-[0.4em] text-[#C5A059] mb-4 font-sans">{day.label}</h4>
                        <p className="text-[#121212] text-[14px] font-sans font-black mb-4 tracking-widest uppercase font-sans">{day.date}</p>
                        ...
                        {day.highlight && <div className="mt-12 pt-12 border-t border-slate-50 font-sans"><p className="text-[11px] font-black text-slate-800 uppercase tracking-[0.2em] leading-relaxed font-sans">Continuous Cinema Unit<br/>Full Day Marathon Production</p></div>}
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <section className="max-w-[1440px] mx-auto py-40 md:py-64 px-8 leading-normal font-serif">
                <div className="text-center mb-32 md:mb-48 font-serif">
                  ...
                  <div className="flex items-center justify-center gap-10 text-[11px] font-sans font-black text-slate-400 tracking-[0.6em] uppercase leading-none font-sans">
                ...
                <div className={`grid gap-8 md:gap-12 items-stretch justify-center font-sans ${proposalData.packages.filter(p => p.isVisible).length === 1 ? 'max-w-2xl mx-auto' : proposalData.packages.filter(p => p.isVisible).length === 2 ? 'md:grid-cols-2 max-w-6xl mx-auto' : 'lg:grid-cols-3'}`}>
                  {proposalData.packages.filter(p => p.isVisible).map((item) => (
                    <div key={item.id} className={`relative flex flex-col p-10 md:p-14 rounded-[3.5rem] border transition-all duration-1000 ${item.isHighlighted ? 'bg-white border-[#C5A059]/40 lg:scale-105 z-10 shadow-2xl' : 'bg-white border-slate-100'} font-sans`}>
                      {item.isHighlighted && <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-[#C5A059] text-white px-12 py-3.5 rounded-full text-[10px] font-black font-sans tracking-[0.5em] shadow-xl leading-none uppercase font-sans">Recommended</div>}
                      <div className="mb-14 md:mb-16 font-sans">
                        <h3 className="text-3xl md:text-4xl font-light mb-6 tracking-tight text-slate-950 leading-none font-serif font-sans">{item.name}</h3>
                      ...
                      <div className="flex-grow space-y-7 md:space-y-8 mb-16 border-t border-slate-50 pt-16 font-sans">
                        {item.features.filter(f => f.trim() !== "").map((feature, fIdx) => (
                          <div key={fIdx} className="flex items-start gap-5 group font-sans">
                            <div className={`mt-1.5 flex-shrink-0 transition-all duration-300 ${item.isHighlighted ? 'text-[#C5A059]' : 'text-slate-300 group-hover:text-slate-950'} font-sans`}><CheckCircle size={20} strokeWidth={1.5} /></div>
                            <span className="text-base md:text-lg text-[#333333] leading-snug tracking-tight font-medium font-sans">{feature}</span>
                          </div>
                        ))}
                      </div>
                      <button onClick={() => openWhatsApp(`Hi! I'd like to secure our date for the ${item.name} Story.`)} className="w-full py-7 rounded-[2rem] font-black text-[11px] font-sans tracking-[0.4em] uppercase transition-all shadow-xl active:scale-95 flex items-center justify-center gap-4 bg-[#121212] text-white hover:bg-[#C5A059] font-sans">Inquire Selection <MessageCircle size={18} /></button>
                    </div>
                  ))}
                </div>
              </section>
              ...
              <section className="max-w-6xl mx-auto py-32 md:py-48 px-8 font-sans">
                <div className="text-center mb-24 font-serif font-sans">
                  <h2 className="text-4xl md:text-6xl font-light mb-8 font-serif tracking-tight text-slate-950 font-serif">The Process</h2>
                  <p className="text-[11px] font-black text-slate-400 tracking-[0.5em] uppercase font-sans">A Sequence of Excellence</p>
                </div>
                <div className="grid md:grid-cols-3 gap-12 relative font-sans">
                  ...
                  {[
                    ...
                    <div key={idx} className="relative z-10 text-center flex flex-col items-center group font-sans">
                      <div className="w-20 h-20 bg-white border border-slate-100 rounded-full flex items-center justify-center mb-8 shadow-xl text-[#C5A059] group-hover:bg-[#C5A059] group-hover:text-white transition-all duration-500 font-sans">{item.icon}</div>
                      <span className="text-[10px] font-black text-[#C5A059] mb-4 uppercase tracking-[0.4em] font-sans">{item.step}</span>
                      <h4 className="text-2xl font-serif italic mb-4 text-slate-950 leading-none font-serif">{item.title}</h4>
                      <p className="text-sm leading-relaxed text-slate-500 font-medium px-4 font-sans">{item.desc}</p>
                    </div>
                  ))}
                </div>
              </section>
                    ...
                    <div className="grid sm:grid-cols-2 gap-8 font-sans">
                      {proposalData.workLinks.map((link) => (
                        <a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer" className="bg-white/5 border border-white/10 p-8 md:p-10 rounded-[2.5rem] flex items-center justify-between group hover:bg-white/10 transition-all duration-500 font-sans">
                          <div className="font-sans pr-4 font-sans">
                            ...
                            <div className="flex items-center gap-2 mb-1 font-sans">
                                ...
                                {link.note && (
                                    <div className="flex items-center gap-1.5 bg-[#C5A059]/10 text-[#C5A059] px-2 py-1 rounded text-[10px] font-black uppercase tracking-widest leading-none font-sans">
              ...
              {/* Luxury FAQs */}
              <section className="max-w-4xl mx-auto py-32 md:py-56 px-8 font-sans font-sans">
                <div className="text-center mb-24 font-serif font-sans">
                  <h2 className="text-4xl md:text-6xl font-serif italic mb-8 text-slate-950 font-serif font-sans">The Fine Print</h2>
                  <p className="text-[11px] font-black text-slate-400 tracking-[0.5em] uppercase font-sans font-sans">Understanding our Studio Commitment</p>
                </div>
                <div className="space-y-12 font-sans font-sans">
                  {[
                    ...
                    <div key={idx} className="group border-b border-slate-100 pb-12 last:border-0 font-sans font-sans">
                      <div className="flex gap-8 items-start font-sans font-sans">
                        <div className="w-12 h-12 rounded-2xl bg-slate-50 text-[#C5A059] flex items-center justify-center shrink-0 font-sans font-sans"><HelpCircle size={20} className="font-sans" /></div>
                        <div className="font-sans font-sans">
                          <h4 className="text-xl font-bold text-slate-950 mb-4 font-sans font-sans font-sans">{faq.q}</h4>
                          <p className="text-slate-600 leading-relaxed font-medium font-sans font-sans font-sans font-sans">{faq.a}</p>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              {proposalData.reviews && proposalData.reviews.length > 0 && (
                <section className="bg-[#fafaf9] py-32 md:py-48 px-8 border-y border-slate-100 font-sans font-sans">
                  <div className="max-w-6xl mx-auto font-sans font-sans font-sans">
                    <div className="text-center mb-24 md:mb-32 font-serif font-sans font-sans font-sans">
                      <h2 className="text-4xl md:text-6xl font-serif italic mb-8 text-slate-950 font-serif font-sans font-sans">Kind Words</h2>
                      <div className="flex items-center justify-center gap-2 mb-4 font-sans font-sans">
                        {[...Array(5)].map((_, i) => <Star key={i} size={16} fill="#C5A059" className="text-[#C5A059] font-sans font-sans" />)}
                      </div>
                      <p className="text-[11px] font-black text-slate-400 tracking-[0.5em] uppercase font-sans font-sans font-sans">Trusted by our Spark Couples</p>
                    </div>
                    <div className="grid md:grid-cols-2 gap-10 md:gap-14 font-sans font-sans">
                      {proposalData.reviews.map((review) => (
                        <div key={review.id} className="relative p-12 md:p-16 bg-white rounded-[3rem] border border-slate-50 shadow-sm group font-sans">
                          <Quote className="absolute top-10 left-10 text-slate-50 group-hover:text-slate-100 transition-colors font-sans font-sans" size={80} strokeWidth={0.5} />
                          <div className="relative z-10 font-sans font-sans leading-relaxed">
                            <p className="text-lg md:text-xl text-[#333333] leading-[1.8] italic font-medium mb-10 font-serif font-sans font-sans">"{review.text}"</p>
                            <div className="flex items-center gap-4 border-t border-slate-50 pt-8 font-sans font-sans">
                              <div className="h-1px w-12 bg-[#C5A059] font-sans font-sans"></div>
                              <p className="font-black text-[12px] uppercase tracking-[0.3em] text-[#C5A059] font-sans font-sans">{review.author}</p>
                            </div>
                          </div>
                        </div>
                      ))}
              ...
              {/* Final Footer */}
              <section className="bg-[#0a0a0a] text-white py-40 md:py-64 px-8 overflow-hidden relative font-sans font-sans">
                <div className="absolute top-0 left-0 w-full h-full opacity-10 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] font-sans font-sans"></div>
                <div className="max-w-6xl mx-auto relative z-10 font-sans font-sans leading-relaxed font-sans">
                  <div className="grid md:grid-cols-2 gap-24 md:gap-48 items-center font-sans font-sans">
                    <div className="text-left font-sans font-sans font-sans leading-relaxed">
                      <h3 className="text-5xl md:text-8xl mb-12 italic leading-[1] text-white font-light tracking-tighter font-serif font-sans font-sans">Your Legacy<br/>Starts Here.</h3>
                      <p className="text-xl md:text-2xl opacity-80 mb-20 font-light leading-relaxed max-w-md text-slate-100 font-sans font-sans">The overture of your family's shared history. We are honored to preserve every chapter.</p>
                      <div className="space-y-16 font-sans font-sans">
                        <div className="flex gap-8 font-sans font-sans">
                          <div className="w-16 h-16 rounded-[1.5rem] border border-white/20 flex items-center justify-center shrink-0 shadow-lg shadow-black/50 font-sans font-sans font-sans"><Clock size={28} className="text-[#C5A059] font-sans font-sans" strokeWidth={1} /></div>
                          <div className="font-sans font-sans"><h4 className="font-black text-[11px] tracking-[0.4em] font-sans font-sans uppercase mb-4 text-[#C5A059] font-sans font-sans">Duration</h4><p className="text-xl font-sans font-bold text-white tracking-tight font-sans font-sans font-sans">Active for 30 days — Valid until {new Date((proposalData.createdAt || Date.now()) + (EXPIRY_DAYS * 24 * 60 * 60 * 1000)).toLocaleDateString()}</p></div>
                        </div>
                        <div className="flex gap-8 font-sans font-sans">
                          <div className="w-16 h-16 rounded-[1.5rem] border border-white/20 flex items-center justify-center shrink-0 shadow-lg shadow-black/50 font-sans font-sans font-sans"><Award size={28} className="text-[#C5A059] font-sans font-sans" strokeWidth={1} /></div>
                          <div className="font-sans font-sans"><h4 className="font-black text-[11px] tracking-[0.4em] font-sans font-sans font-sans uppercase mb-4 text-[#C5A059] font-sans font-sans font-sans">Booking Contract</h4><p className="text-xl font-sans font-bold text-white tracking-tight font-sans font-sans font-sans">A signed agreement formalizes all discussed timelines and investment details.</p></div>
                        </div>
                      </div>
                    </div>
                    <div className="bg-white text-slate-950 p-12 md:p-24 rounded-[4rem] md:rounded-[6rem] shadow-[0_80px_150px_-30px_rgba(0,0,0,0.8)] relative group overflow-hidden font-sans font-sans">
                      <div className="absolute top-0 right-0 w-64 h-64 bg-[#C5A059]/5 rounded-bl-[10rem] transition-transform duration-[2s] group-hover:scale-125 font-sans font-sans font-sans"></div>
                      <h4 className="text-4xl md:text-6xl font-serif mb-10 italic leading-none font-serif font-sans font-sans">Vision Call</h4>
                      <p className="text-slate-600 mb-16 text-lg md:text-2xl leading-relaxed font-sans font-medium pr-4 font-sans font-sans">To ensure our artistic styles align, we invite you to a brief 15-minute introductory session.</p>
                      <button onClick={() => openWhatsApp(`Hi Spark Studios! We'd like to schedule a Vision Call for the ${proposalData.clientName} celebration.`)} className="w-full bg-[#C5A059] text-white py-8 rounded-[2.5rem] font-black text-[11px] uppercase tracking-[0.5em] font-sans font-sans font-sans hover:bg-slate-950 transition-all shadow-2xl shadow-[#C5A059]/40 active:scale-95 flex items-center justify-center gap-4 font-sans font-sans">Connect on WhatsApp <MessageCircle size={20} className="font-sans font-sans" /></button>
                    </div>
                  </div>
                  <div className="mt-48 md:mt-64 pt-20 border-t border-white/10 text-center font-sans font-sans"><p className="text-[11px] uppercase tracking-[1em] opacity-40 font-sans font-black leading-relaxed font-sans font-sans font-sans">The Spark Studios &copy; 2026 — Fine Art Cinematography</p></div>
                </div>
              </section>
            </>
          )}
